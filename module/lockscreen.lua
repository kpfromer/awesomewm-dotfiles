...
local locker_config = {
  ...
  face_capture_dir = config.module.lockscreen.face_capture_dir or
      '$(xdg-user-dir PICTURES)/Intruders/',
...
if not locker_config.military_clock then clock_format = '<span font="Inter Bold 52">%I:%M %p</span>' end
...
local locker = function(s)
  ...
  local update_profile_pic = function()
    awful.spawn.easy_async_with_shell(
        apps.utils.update_profile, function(stdout)
          stdout = stdout:gsub('%\n', '')
  ...
  local check_caps = function()
    awful.spawn.easy_async_with_shell(
        'xset q | grep Caps | cut -d: -f3 | cut -d0 -f1 | tr -d \' \'', function(stdout)
  ...
  local check_webcam = function()
    awful.spawn.easy_async_with_shell(
        'ls -l /dev/video* | grep /dev/video0', function(stdout)
    ...
    keybindings = {
      ...
      awful.key {
        ...
        on_press = function(self)
          if not type_again then return end
          self:stop()
  ...
  local free_keygrab = function()
    ...
    if keygrabbing_instance then keygrabbing_instance:stop() end
  ...
  awesome.connect_signal(
      'module::lockscreen_show', function()
        if lock_again == true or lock_again == nil then
...
local check_lockscreen_visibility = function()
  ...
  if focused.lockscreen and focused.lockscreen.visible then return true end
  if focused.lockscreen_extended and focused.lockscreen_extended.visible then return true end
  return false
end

-- Notifications signal
naughty.connect_signal(
    'request::display', function(_)
      if check_lockscreen_visibility() then naughty.destroy_all_notifications(nil, 1) end
    end
)
...
local filter_bg_image = function(wall_name, index, ap, width, height)
  ...
  if locker_config.blur_background then blur_filter_param = '-filter Gaussian -blur 0x10' end

  -- Create imagemagick command
  local magic = [[
	sh -c "
	if [ ! -d ]] .. locker_config.tmp_wall_dir .. [[ ]; then mkdir -p ]] .. locker_config.tmp_wall_dir ..
                    [[; fi

	convert -quality 100 -brightness-contrast -20x0 ]] .. ' ' .. blur_filter_param .. ' ' ..
                    locker_config.wall_dir .. wall_name .. [[ -gravity center -crop ]] .. ap ..
                    [[:1 +repage -resize ]] .. width .. 'x' .. height .. [[! ]] ..
