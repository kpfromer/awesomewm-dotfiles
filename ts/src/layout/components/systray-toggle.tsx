...
export const SystemTrayToggle: JSX.FunctionComponent = () => {
  const tray = wibox.widget(<SystemTray screen="primary" base_size={dpi(20)} horizontal />);
  tray.visible = false;
      ...
      <PanelOutline>
        <Clickable
          onButtonPress={(button) => {
            ...
            if (button === 1) {
              if (tray.visible) {
                icon.set_image(gears.surface.load_uncached(`${widgetIconDir}right-arrow.svg`)!);
              } else {
                icon.set_image(gears.surface.load_uncached(`${widgetIconDir}left-arrow.svg`)!);
              }
